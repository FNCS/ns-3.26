...
main (int argc, char *argv[])
{
    ...
    NS_LOG_INFO("Start time: " << ts.tv_sec);
    ...
    if (enbDensity == 0){
        ...
        numEnb = enbDensity * enbDensity;
    }
    
    NS_LOG_INFO("Number of eNodeBs: " << numEnb);
    ...
    double centerX = xRange/2;
    double centerY = yRange/2;
...
//     double y2 = centerY+(centerY/2);
    ...
    std::vector<double> ENBx;
    std::vector<double> ENBy;
    // Uniformly spaced eNodeBs require a lot for good coverage
    /*
    for (int rowIdx = 0; rowIdx < enbDensity; rowIdx++){
        for (int colIdx = 0; colIdx < enbDensity; colIdx++){
            ENBx.push_back(minX+((rowIdx+1)*(xRange/(enbDensity+1))));
            ENBy.push_back(minY+((colIdx+1)*(yRange/(enbDensity+1))));
            double eNBCount = (enbDensity * rowIdx) + colIdx + 1;
            NS_LOG_INFO("Added eNB number " << eNBCount << "/" << numEnb << " at: " << ENBx.back() << ", " <<ENBy.back());
        }
    }
    */
    //Randomly locating them very close to smart meters
    for (int idx = 0; idx < numEnb; idx ++){
        int randIdx = std::rand() % numMeters;
        ENBx.push_back(nodeX[randIdx] + 10);
        ENBy.push_back(nodeY[randIdx] + 10);
        NS_LOG_INFO("Added eNB number " << idx+1 << "/" << numEnb << " at: " << ENBx.back() << ", " <<ENBy.back());
    }
        
    Ptr<ListPositionAllocator> positionAllocENB = CreateObject<ListPositionAllocator> ();
    for (int idx = 0; idx < numEnb; idx ++){
        positionAllocENB->Add (Vector (ENBx[idx], ENBx[idx], 5));
    }
    ...
    */
    ...
    //Necessary in LTE simulations due to constantly generated LTE maintenance events.
    ...
    ts.tv_sec = mts.tv_sec;
    ts.tv_nsec = mts.tv_nsec;
    NS_LOG_INFO("Stop time: " << ts.tv_sec);
    return 0;
}
